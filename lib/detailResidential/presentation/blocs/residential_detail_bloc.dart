...
on<GenerateGuestCode>((event, emit) async {
  try {
    final qrBase64 = await repository.generateGuestCode(
      event.residenciaId,
      event.usos,
    );

    final current = state as ResidentialDetailLoaded;
    final updated = current.residential.copyWith(qrBase64: qrBase64);

    emit(ResidentialDetailLoaded(updated));
  } catch (e) {
    emit(ResidentialDetailError("No se pudo generar el código"));
  }
});
